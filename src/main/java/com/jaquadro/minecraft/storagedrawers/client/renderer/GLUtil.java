...
public final class GLUtil {
    ...
    public static void saveGLState(List<int[]> state, int[] saveList) {
        if (state == null || saveList == null) return;

        for (int i = state.size(), n = saveList.length; i < n; i++) state.add(new int[2]);

        for (int i = 0, n = state.size(); i < n; i++) state.get(i)[0] = 0;
    ...
    public static void restoreGLState(List<int[]> state) {
        ...
        for (int i = 0, n = state.size(); i < n; i++) {
            ...
            if (entry[1] == 0) GL11.glDisable(entry[0]);
            else GL11.glEnable(entry[0]);
        }
    }
}
