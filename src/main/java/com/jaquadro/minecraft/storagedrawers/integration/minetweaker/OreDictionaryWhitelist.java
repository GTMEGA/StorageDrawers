...
public class OreDictionaryWhitelist {
    ...
    public static void add(String name) {
        ...
        else MineTweakerAPI.logError("Tried to add empty ore dictionary name to drawer conversion whitelist.");
    }
    ...
    private static class AddNameAction implements IUndoableAction {
        ...
        @Override
        public void undo() {
            if (added) StorageDrawers.oreDictRegistry.removeWhitelist(name);
        }

        @Override
        public String describe() {
            if (added) return "Adding ore dictionary key '" + name + "' to drawer conversion whitelist.";
            else return "Ore dictionary key '" + name + "' already registered in drawer conversion whitelist.";
        }
